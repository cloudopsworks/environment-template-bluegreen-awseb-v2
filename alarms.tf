...
locals {
  tags_global = merge(var.extra_tags, {
    Environment = format("%s-%s", var.environment_name, var.namespace)
    Namespace   = var.namespace
  })
  tags_a = merge(var.extra_tags, {
    Environment        = format("%s-%s", var.environment_name, var.namespace)
    Namespace          = var.namespace
    Release            = var.release_name_a
    Deployment_Traffic = "a"
  })
  tags_b = merge(var.extra_tags, {
    Environment        = format("%s-%s", var.environment_name, var.namespace)
    Namespace          = var.namespace
    Release            = var.release_name_b
    Deployment_Traffic = "b"
  })
}
...
resource "aws_cloudwatch_metric_alarm" "metric_alarm_a" {
  ...
  alarm_name          = format("MetricsAlarm-%s-%s-%s-%s", var.region, var.release_name_a, var.namespace, "a")
...
resource "aws_cloudwatch_metric_alarm" "metric_alarm_b" {
  ...
  alarm_name          = format("MetricsAlarm-%s-%s-%s-%s", var.region, var.release_name_b, var.namespace, "b")
  comparison_operator = "GreaterThanThreshold"
  statistic           = "Maximum"
  threshold           = var.cloudwatch_alarm_threshold
  period              = var.cloudwatch_alarm_period
